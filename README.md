...
### -포트 매핑
**1.포트 매핑이 필요한 이유**
도커 컨테이너는 **외부와 완전히 차단된 '밀실'**과 같습니다.

* **컨테이너 내부**: 웹 서버가 **80번 문**을 열고 기다리고 있습니다.
* **내 컴퓨터(외부)**: 컨테이너가 벽으로 막혀 있어 80번 문으로 직접 찾아갈 수 없습니다.

따라서 **바깥 세상(내 컴퓨터)과 내부(컨테이너)를 이어주는 '전용 연결 통로'**를 만들어 주는 것이 바로 **포트 매핑**입니다.
