...
### -절대 경로와 상대 경로
**1. 절대 경로**
루트(/)부터 시작하는 전체 경로 (예: /Users/user/app/index.html). 위치에 상관없이 항상 동일.

**2. 상대 경로**
현재 작업 디렉토리(.) 기준으로의 경로 (예: ./app/index.html). 현재 위치에 따라 가리키는 대상이 달라짐.
...
| 표기 | 소유자 (User) | 그룹 (Group) | 기타 사용자 (Other) |
| :---: | :---: | :---: | :---: |
...
Dockerfile 작성  
```
# 1. 기존 공식 이미지를 기반으로 시작 (베이스 이미지)
FROM <베이스_이미지_이름>:<태그>

# 2. 필요한 패키지 설치 (커스텀 설정)
RUN apt-get update && apt-get install -y <설치할_패키지명_1> <설치할_패키지명_2>

# 3. 내 소스코드 및 설정 파일 복사
COPY <내_컴퓨터_파일_경로> <컨테이너_내부_경로>

# 4. 컨테이너 실행 시 동작할 기본 명령어 설정
CMD ["<실행할_프로그램>", "<옵션1>", "<옵션2>"]
```
커스텀 이미지 빌드 명령어
```
docker build -t <이미지이름>:<태그버전> <Dockerfile_위치_경로>
```
...
### -포트 매핑
**1.포트 매핑이 필요한 이유**
컨테이너는 격리된 가상 네트워크를 사용하므로, 호스트(내 컴퓨터)의 포트와 컨테이너의 포트를 연결(-p host:container)해야 외부 브라우저에서 컨테이너 내부 서비스로 접근할 수 있음.
...
### -Docker 볼륨(영속 데이터)
**1.Docker 볼륨**
컨테이너가 삭제되어도 **데이터(DB, 파일 등)를 내 컴퓨터에 안전하게 남겨두기 위한 기능**입니다.

**2.사용 방법**
```bash
docker run -v <내_컴퓨터_경로>:<컨테이너_경로> <이미지_이름>
```
